...
def test_translated_strings_keep_english_placeholders() -> None:
    """Test that all translated strings keep the same placeholders as the English strings."""
    ...
    for translation_file in TRANSLATIONS_DIR.glob("*.json"):
        ...
        for path, translated_value in translated_strings.items():
            ...
            assert _placeholders(translated_value) == _placeholders(english_value), (
...
def _load_translation(file_name: str) -> dict[str, object]:
    return cast(
        dict[str, object], json.loads((TRANSLATIONS_DIR / file_name).read_text())
    )
