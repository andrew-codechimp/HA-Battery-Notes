...
| `battery_level` | `float` | Battery level % of the device (not supplied for templated low sensors). |
| `previous_battery_level` | `float` | Previous battery level % of the device (not supplied for templated low sensors). |
| `scheduled` | `bool` | Returns true if the event was raised by a schedule service call, false if it's from a device event. |
