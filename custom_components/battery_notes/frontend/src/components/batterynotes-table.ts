...
export type TableData =
  | Record<string, any>
  | { id: string | number; warning?: boolean };
...
@customElement("batterynotes-table")
export class BatteryNotesTable extends LitElement {
  @property()
  hass!: HomeAssistant;
  ...
  render() {
    ...
    const filteredData = this.data.filter((e) =>
      this.filterTableData(e, this.filterConfig),
    );
    return html`
  ...
  renderHeaderRow() {
    ...
    return html`
      ...
      e.hide
        ...
        : html`
                <div
                  class="table-cell ${e.text ? "text" : ""} ${e.grow
            ? "grow"
            : ""} ${e.align ? e.align : ""}"
  ...
  renderDataRow(data: TableData) {
    ...
    return html`
      <div
        class="table-row ${this.selectable ? "selectable" : ""} ${data.warning
        ? "warning"
        : ""}"
  ...
  filterTableData(
    data: TableData,
    filterConfig?: Record<string, { value: string[] }>,
  ) {
  ...
  private _getFilteredItems() {
    return this.data!.filter((e) => !this.filterTableData(e, this.filterConfig))
      .length;
  }
    ...
    return html`
      <div class="table-filter">
        ...
        <mwc-menu
          .corner=${"BOTTOM_START"}
          .fixed=${true}
          @closed=${this._applyFilterSelection}
        >
        ...
        ? html`
              <alarmo-chip
                cancellable
                table
                @button-clicked=${this._clearFilters}
              >
                ${localize(
        ...
        items = items.map(e => {
          if (e.badge && typeof e.badge == "function")
            return {
              ...
              badge: e.badge(
                this.data?.filter((a) =>
                  this.filterTableData(a, omit(this.filterSelection!, key)),
                ),
              ),
            };
          else return e;
