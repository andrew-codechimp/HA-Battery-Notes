...
import "../../components/batterynotes-table";
import { exportPath } from "../../common/navigation";

const noArea = "no_area";

@customElement("devices-overview-card")
    ...
    return html`
      <ha-card header="${localize("panels.sensors.title", this.hass.language)}">
        <div class="card-content">
          ${localize(
      "panels.sensors.cards.sensors.description",
      this.hass.language,
    )}
        </div>
        ...
        navigate(
          this,
          exportPath("sensors", { params: { edit: ev.detail.id } }),
          true,
        );
      }}
        >
          ${localize(
        "panels.sensors.cards.sensors.table.no_items",
        this.hass.language,
      )}
        </alarmo-table>
      </ha-card>
  ...
  private tableColumns(): Dictionary<TableColumn> {
    const warningTooltip = () => html`
      <simple-tooltip animation-delay="0">
        ${localize(
      "panels.sensors.cards.sensors.table.no_area_warning",
      this.hass.language,
    )}
      </simple-tooltip>
    `;
    ...
    return {
      icon: {
        ...
        renderer: (data: AlarmoSensor) => {
          ...
          const type = Object.keys(ESensorTypes).find(
            (e) => ESensorTypes[e] == data.type,
          ) as ESensorTypes;
          const icon = stateObj
            ? stateObj.state === "on"
      ...
      modes: {
        title: localize(
          "panels.sensors.cards.sensors.table.arm_modes",
          this.hass.language,
        ),
        width: '25%',
        hide: this.narrow,
        ...
        renderer: (data: AlarmoSensor) => html`
          ...
          <span class="${!data.enabled ? "disabled" : ""}">
            ${data.always_on
            ? localize(
              "panels.sensors.cards.sensors.table.always_on",
              this.hass!.language,
            )
            : data.modes.length
              ? data.modes
                .map((e) =>
                  localize(`common.modes_short.${e}`, this.hass!.language),
                )
                .join(", ")
              : this.hass.localize(
  ...
  private getTableData(): Record<string, any>[] {
    const sensorsList = Object.keys(this.sensors).map(id => {
      ...
      const res: TableData & { name: string } = {
        ...
        modes: config.always_on
          ? modesList
          : config.modes.filter((e) => modesList.includes(e)),
        warning: !config.area,
  ...
  private getTableFilterOptions() {
    let areaFilterOptions = Object.values(this.areas)
      .map((e) =>
        Object({
          value: e.area_id,
          name: e.name,
          badge: (list: AlarmoSensor[]) =>
            list.filter((item) => item.area == e.area_id).length,
        })
      )
      ...
      areaFilterOptions = [
        {
          ...
          badge: (list: AlarmoSensor[]) =>
            list.filter((item) => item.area == noArea).length,
        },
        ...areaFilterOptions,
      ];
    ...
    const modeFilterOptions = getModesList(this.areas).map(e =>
      Object({
        ...
        badge: (list: AlarmoSensor[]) =>
          list.filter((item) => item.modes.includes(e)).length,
      })
    );
    ...
    const filterConfig: TableFilterConfig = {
      area: {
        name: localize(
          ...
          "name",
          localize(
            "panels.actions.cards.new_action.fields.area.heading",
            this.hass.language,
          ),
        ),
