...
export class ConfirmationDialog extends LitElement {
  ...
  render() {
    ...
    return html`
      <ha-dialog open .heading=${true} @closed=${this.cancelAction} @close-dialog=${this.cancelAction}>
        <div slot="heading">
          <ha-header-bar>
            <ha-icon-button slot="navigationIcon" dialogAction="cancel" .path=${mdiClose}></ha-icon-button>
            <span slot="title">${this.hass.localize('dialogs.confirmation.title')}</span>
          </ha-header-bar>
        </div>
        <div class="wrapper">${this._params.message || ''}</div>

        <mwc-button slot="primaryAction" style="float: left" @click=${this.confirmAction} dialogAction="close">
          ${this.hass.localize('dialogs.generic.ok')}
        </mwc-button>
        <mwc-button slot="primaryAction" style="float: left" @click=${this.cancelAction} dialogAction="cancel">
